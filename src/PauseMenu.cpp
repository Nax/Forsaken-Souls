...
PauseMenu::load()
{
	_resumeBatch.draw(lm::FontProvider::instance().get("roboto80"), "Resume",
					{SCREEN_WIDTH / 2 - 200.f / 2.f, SCREEN_HEIGHT / 2 - 300.f / 2.f}, {1.f, 0.f, 1.f, _resumeAlpha});
	_resumeBatch.send();

	_settingsBatch.draw(lm::FontProvider::instance().get("roboto80"), "Settings",
					{SCREEN_WIDTH / 2 - 200.f / 2.f, SCREEN_HEIGHT / 2 - 100.f / 2.f}, {1.f, 0.f, 1.f, _settingsAlpha});
	_settingsBatch.send();
